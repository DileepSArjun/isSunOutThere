...
export class WeatherService {
  ...
  static appID: string = '952d6b1a52fe15a7b901720074680562';
  ...
  getCityWeatherByName(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<string> {
    ...
    this.http
      .get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=${WeatherService.appID}`)
      .subscribe((data) => {
  ...
  getCitiesWeathersByNames(cities: Array<string>, metric: 'metric' | 'imperial' = 'metric'): Subject<any> {
    ...
    cities.forEach((city) => {
      citiesSubject.next(this.http.get(
        `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=${WeatherService.appID}`));
    });
  ...
  getWeatherState(city: string): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=${WeatherService.appID}`)
      .subscribe((data) => {
  ...
  getMaxTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number>  {
    ...
    let max: number;
    this.http.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=${metric}&APPID=${WeatherService.appID}`)
      .subscribe((weather: any) => {
